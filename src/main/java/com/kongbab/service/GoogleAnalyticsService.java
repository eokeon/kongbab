...
public class GoogleAnalyticsService {
    ...
    @Value("${google.analytics.credentials-path:classpath:kongbab-visit.json}")
    ...
    private synchronized String getAccessToken() throws Exception {
        if (cachedCredentials == null) {
            ...
            if (is == null) {
                File directFile = new File("src/main/resources/kongbab-visit.json");
                if (directFile.exists()) {
                    is = new FileInputStream(directFile);
                } else {
                    File rootFile = new File("kongbab-visit.json");
                    if (rootFile.exists()) {
    ...
    public Map<String, Object> getAnalyticsSummary(boolean forceRefresh) {
        ...
        synchronized (summaryLock) {
            ...
            try {
                ...
                result.put("hint", "구글 애널리틱스 관리자 페이지에서 서비스 계정(konbab-reader@kongbab.iam.gserviceaccount.com)이 뷰어로 추가되었는지 확인해 주세요.");
            }
