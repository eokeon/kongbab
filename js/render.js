/**
 * 콩밥특별시 GTA RP 아카이브 - UI 렌더링 및 네비게이션 (render.js)
 * 
 * - 상단 어드민/게스트 헤더 바 및 카테고리 탭
 * - 6대 카테고리 (경찰, EMS, 갱단, 사업체, 기자, 시민) 화면 렌더링
 * - 갱단(12개) 및 사업체(4개) 하위 조직 및 인원 목록 렌더링
 * - 유튜브 영상 그리드 (편집 영상 vs 풀 영상 탭 분리)
 * - 통합 검색 결과 렌더링
 * - 네비게이션 및 이벤트 리스너
 */

// 상단 우측 로그인/사용자 상태 바 렌더링
function renderHeaderAuth() {
  const container = document.getElementById("header-auth");
  if (!container) return;

  if (isAdmin()) {
    const backupCount = typeof getStoredBackups === "function" ? getStoredBackups().length : 0;
    container.innerHTML = `
      <div class="flex items-center gap-2">
        <div class="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-950/70 border border-amber-600/50 text-amber-300 text-xs font-bold shadow-sm">
          <span class="w-2 h-2 rounded-full bg-amber-400 animate-pulse"></span>
          <span>🛡️ 어드민 (admin)</span>
        </div>
        <button 
          onclick="openBackupModal()" 
          class="px-3 py-1.5 rounded-xl bg-red-950/80 hover:bg-red-900 border border-red-600/60 text-red-200 hover:text-white text-xs font-bold transition-all shadow-sm flex items-center gap-1.5 cursor-pointer"
          title="JSON 백업 & 복구 관리자 (최대 20개 자동 롤링 보관)"
        >
          <span>📦</span>
          <span id="backup-badge-text">백업/복구 (${backupCount}/20)</span>
        </button>
        <button 
          onclick="logoutUser()" 
          class="px-3 py-1.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-white text-xs font-semibold border border-zinc-800 transition-colors cursor-pointer"
          title="로그아웃"
        >
          로그아웃
        </button>
    ...
    container.innerHTML = `
      <div class="flex items-center gap-2">
        ...
        <button 
          ...
          class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white text-xs font-bold transition-all shadow-md shadow-red-600/25 cursor-pointer"
          title="로그인 (어드민 / 게스트)"
        >
...
// 영상 탭 변경 함수 (편집 영상 'clip' vs 풀 영상 'full')
function setVideoTab(tab) {
  state.currentVideoTab = tab;
  const container = document.getElementById("main-content");
  if (container && state.currentMember) {
    renderMemberVideos(container);
  }
}

// 대분류 카테고리 탭 렌더링 (경찰, EMS, 갱단, 사업체, 기자, 시민)
function renderCategoryTabs() {
  ...
  tabContainer.innerHTML = KONGBAB_DATA.categories.map(cat => {
    ...
    // 뱃지 숫자 (갱단/사업체는 조직 수, 나머지는 인원 수)
    const countLabel = cat.hasSubgroups 
      ? `${cat.groups.length}개` 
    ...
    return `
      <button 
        ...
        class="flex items-center gap-2 px-3.5 py-2.5 rounded-xl font-semibold text-sm transition-all duration-200 cursor-pointer ${activeClass}"
      >
        <span>${cat.emoji || ''}</span>
        <span>${cat.name}</span>
...
function renderContent() {
  ...
  // 3. 경찰, EMS, 기자, 시민 -> 하위 조직 선택 없이 바로 인원 표시
  if (!cat.hasSubgroups) {
  ...
  // 4. 갱단, 사업체
  // 4-1. 특정 갱/사업체 누름 -> 소속 인원 표시
  if (state.currentGroup) {
    renderGroupMembers(mainContent);
    return;
  }

  // 4-2. 갱단/사업체 기본 화면 -> 12개 갱 / 4개 사업체 카드 목록 표시
  renderSubgroupList(mainContent, cat);
}

// [경찰, EMS, 기자, 시민] 하위 조직 없이 바로 인원 목록 표시 (드래그 앤 드롭 순서 변경 지원)
function renderDirectCategoryMembers(container, cat) {
  ...
  let membersHtml = members.map(member => {
    return `
      <div 
        draggable="true"
        data-drag-type="direct-member"
        data-drag-id="${member.id}"
        ondragstart="handleCardDragStart(event, 'direct-member', '${member.id}')"
        ondragover="handleCardDragOver(event)"
        ondragleave="handleCardDragLeave(event)"
        ondrop="handleCardDrop(event, 'direct-member', '${member.id}')"
        ondragend="handleCardDragEnd(event)"
        onclick="selectDirectMember('${member.id}')"
      ...
      >
        <div>
          <div class="flex items-start gap-4 mb-4">
            <div class="relative">
              ...
              />
              ${member.role ? `
              <span class="absolute -bottom-1 -right-1 text-[10px] font-bold px-1.5 py-0.5 rounded ${member.badgeColor || 'bg-zinc-800'} text-white shadow">
                ${member.role}
              </span>` : ''}
            </div>
              ...
              <div class="flex items-center justify-between gap-2">
                ...
                ${isAdmin() ? `
                  <div class="flex items-center gap-1.5 ml-2 flex-shrink-0 card-header-actions" onclick="event.stopPropagation()">
                    <button 
                      onclick="openMemberModal('edit', '${member.id}')" 
                      title="인원 수정" 
        ...
        <div class="pt-4 border-t border-zinc-800/60 flex items-center justify-between text-xs text-zinc-400">
          <span class="font-medium text-zinc-300">유튜브 영상 <strong class="text-red-400">${(member.videos || []).length}개</strong></span>
  ...
  if (members.length === 0) {
    membersHtml = `
      <div class="col-span-full py-16 text-center bg-zinc-900/40 rounded-2xl border border-zinc-800/50">
        ...
        <h4 class="text-lg font-bold text-zinc-300">등록된 인원이 없습니다.</h4>
        <p class="text-sm text-zinc-500 mt-1">${isAdmin() ? "상단의 [+ 인원 추가] 버튼으로 새 인원을 등록해보세요." : "등록된 인원이 없습니다."}</p>
      </div>
    `;
  }
  ...
  container.innerHTML = `
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      ...
      ${isAdmin() ? `
        <button 
          onclick="openMemberModal('add', null, '${cat.id}')"
          class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-black text-xs font-bold transition-all shadow-lg shadow-amber-500/20 cursor-pointer self-start sm:self-auto"
          title="${cat.name}에 새 인원 추가"
        >
...
// [갱단 & 사업체] 12개 갱단 또는 4개 사업체 카드 목록 표시 (드래그 앤 드롭 순서 변경 지원)
function renderSubgroupList(container, cat) {
  const theme = COLOR_THEMES[cat.color] || COLOR_THEMES.blue;

  let cardsHtml = cat.groups.map(group => {
...
// [특정 갱단/사업체 선택 시] 소속 인원 목록 표시 (드래그 앤 드롭 순서 변경 지원)
function renderGroupMembers(container) {
  ...
  let membersHtml = group.members.map(member => {
    return `
      <div 
        draggable="true"
        data-drag-type="group-member"
        data-drag-id="${member.id}"
        ondragstart="handleCardDragStart(event, 'group-member', '${member.id}')"
        ondragover="handleCardDragOver(event)"
        ondragleave="handleCardDragLeave(event)"
        ondrop="handleCardDrop(event, 'group-member', '${member.id}')"
        ondragend="handleCardDragEnd(event)"
        onclick="selectGroupMember('${member.id}')"
        class="group bg-zinc-900/80 border border-zinc-800/80 hover:border-zinc-700 rounded-2xl p-5 transition-all duration-300 hover:-translate-y-1.5 cursor-grab active:cursor-grabbing shadow-lg hover:shadow-2xl flex flex-col justify-between select-none"
      >
        <div>
          <div class="flex items-start gap-4 mb-4">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <button 
          ...
          class="inline-flex items-center gap-1.5 text-xs text-zinc-400 hover:text-white bg-zinc-800 hover:bg-zinc-700 px-3 py-1.5 rounded-lg transition-colors mb-3 cursor-pointer"
        >
          ${SVG_ICONS.back}
        ...
        <div class="flex items-center gap-3 flex-wrap">
          ...
          ${isAdmin() ? `
            <button 
              onclick="openMemberModal('add', null, '${cat.id}', '${group.id}')"
              class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-black text-xs font-bold transition-all shadow-lg shadow-amber-500/20 cursor-pointer"
              title="${group.name}에 새 인원 추가"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
              <span>+ 인원 추가</span>
            </button>
      ...
      <div class="bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 flex items-center gap-4 self-start md:self-auto">
        <div class="text-right">
          ...
          <p class="text-xl font-bold text-white">${group.members.length}명</p>
        </div>
        <div class="w-px h-8 bg-zinc-800"></div>
        <div class="text-right">
          <p class="text-xs text-zinc-500">등록된 영상</p>
          <p class="text-xl font-bold text-red-400">${group.members.reduce((s, m) => s + (m.videos || []).length, 0)}개</p>
        </div>
      </div>
    </div>
...
// [영상 목록] 인원 선택 시 유튜브 영상 목록 표시 (편집 영상 / 풀 영상 탭 분리, 드래그 앤 드롭 순서 변경)
function renderMemberVideos(container) {
  ...
  if (!cat.hasSubgroups) {
    backButtonHtml = `
      <button 
        onclick="resetToCategory('${cat.id}')" 
        class="inline-flex items-center gap-1.5 text-xs text-zinc-400 hover:text-white bg-zinc-800 hover:bg-zinc-700 px-3 py-1.5 rounded-lg transition-colors mb-4 cursor-pointer"
      >
        ${SVG_ICONS.back}
        ...
        <div>
          ...
          >
            ...
            <div class="absolute top-2.5 left-2.5 ${isFull ? 'bg-indigo-950/90 text-indigo-300 border-indigo-700/60' : 'bg-red-950/90 text-red-300 border-red-700/60'} border backdrop-blur-md text-[11px] font-bold px-2.5 py-1 rounded-lg shadow flex items-center gap-1 z-10 whitespace-nowrap select-none flex-shrink-0">
              <span class="whitespace-nowrap select-none">${isFull ? '📹 풀 영상' : '🎬 편집 영상'}</span>
            </div>
            ...
            <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 z-10">
              ...
              <span class="text-xs font-bold text-white bg-black/80 px-2.5 py-1 rounded-md flex items-center gap-1 whitespace-nowrap select-none">
                <span class="whitespace-nowrap">유튜브로 이동</span>
                ${SVG_ICONS.external}
              </span>
            </div>
        ...
        ${isAdmin() ? `
          <div class="px-5 py-2.5 bg-zinc-950 border-t border-zinc-800/80 flex items-center justify-between">
            ...
            <div class="flex items-center gap-1.5">
              <button 
                onclick="openVideoModal('edit', '${video.id}')"
                class="px-2.5 py-1 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-semibold transition-colors cursor-pointer"
              >
                ✏️ 수정
              </button>
              <button 
                onclick="deleteVideo('${video.id}')"
                class="px-2.5 py-1 rounded-lg bg-red-950/70 hover:bg-red-900 text-red-300 text-xs font-semibold border border-red-800/50 transition-colors cursor-pointer"
              >
                🗑️ 삭제
              </button>
            </div>
  ...
  if (displayedVideos.length === 0) {
    if (allVideos.length === 0) {
      videosHtml = `
        <div class="col-span-full py-16 text-center bg-zinc-900/40 rounded-2xl border border-zinc-800/50">
          <div class="w-14 h-14 mx-auto rounded-full bg-zinc-800 flex items-center justify-center text-zinc-500 mb-3">
            ${SVG_ICONS.youtube}
          </div>
          <h4 class="text-lg font-bold text-zinc-300">등록된 유튜브 영상이 없습니다.</h4>
          <p class="text-sm text-zinc-500 mt-1">새 유튜브 영상을 등록해보세요.</p>
        </div>
      `;
    } else if (currentTab === "clip") {
      videosHtml = `
        <div class="col-span-full py-16 text-center bg-zinc-900/40 rounded-2xl border border-zinc-800/50">
          <div class="w-14 h-14 mx-auto rounded-full bg-zinc-800 flex items-center justify-center text-zinc-400 mb-3 text-2xl">
            🎬
          </div>
          <h4 class="text-lg font-bold text-zinc-300">등록된 편집 영상이 없습니다.</h4>
          <p class="text-sm text-zinc-500 mt-1">풀 영상 탭에서 다시보기 영상을 확인해보세요.</p>
          ${fullCount > 0 ? `
            <div class="mt-4">
              <button onclick="setVideoTab('full')" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition-colors cursor-pointer inline-flex items-center gap-1.5 shadow-lg shadow-indigo-600/30">
                <span>📹 풀 영상 (${fullCount}개) 보러가기</span>
              </button>
            </div>
          ` : ''}
        </div>
      ...
      videosHtml = `
        <div class="col-span-full py-16 text-center bg-zinc-900/40 rounded-2xl border border-zinc-800/50">
          ...
          <h4 class="text-lg font-bold text-zinc-300">등록된 풀 영상(다시보기)이 없습니다.</h4>
          <p class="text-sm text-zinc-500 mt-1">편집 영상 탭에서 하이라이트 영상을 확인해보세요.</p>
          ${clipCount > 0 ? `
            <div class="mt-4">
              <button onclick="setVideoTab('clip')" class="px-4 py-2 bg-red-600 hover:bg-red-500 text-white rounded-xl text-xs font-bold transition-colors cursor-pointer inline-flex items-center gap-1.5 shadow-lg shadow-red-600/30">
                <span>🎬 편집 영상 (${clipCount}개) 보러가기</span>
              </button>
            </div>
          ` : ''}
        </div>
  ...
  const affiliation = cat.hasSubgroups 
  ...
  container.innerHTML = `
      ...
      <div class="bg-gradient-to-r from-zinc-900 via-zinc-900 to-zinc-950 border border-zinc-800 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-2xl relative overflow-hidden">
        <div class="flex items-center gap-6 relative z-10">
          <img 
            src="${getMemberAvatar(member)}" 
            alt="${member.name}" 
            class="w-24 h-24 md:w-28 md:h-28 rounded-3xl object-cover border-4 border-zinc-800 shadow-2xl"
          />
          <div>
            <div class="flex items-center gap-2.5 mb-1.5 flex-wrap">
              ${member.role ? `
              <span class="text-xs font-bold px-2.5 py-1 rounded-md ${member.badgeColor || 'bg-red-700'} text-white">
                ${member.role}
              </span>` : ''}
              <span class="text-xs font-semibold px-2.5 py-1 rounded-md bg-zinc-800 text-zinc-300">
                소속: ${affiliation}
              </span>
            </div>
            ...
            <p class="text-amber-400 text-base font-semibold mt-1">
              스트리머: ${member.streamer}
            </p>
            ${isAdmin() ? `
              <div class="flex items-center gap-2 mt-3">
                <button 
                  onclick="openMemberModal('edit', '${member.id}')"
                  class="px-3 py-1.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-200 hover:text-white text-xs font-semibold transition-colors cursor-pointer inline-flex items-center gap-1"
                >
                  ✏️ 인원 정보 수정
                </button>
                <button 
                  onclick="deleteMember('${member.id}')"
                  class="px-3 py-1.5 rounded-xl bg-red-950/80 hover:bg-red-900 text-red-300 hover:text-white text-xs font-semibold border border-red-800/50 transition-colors cursor-pointer inline-flex items-center gap-1"
                >
                  🗑️ 인원 삭제
                </button>
              </div>
        ...
        <div class="flex items-center gap-3 self-start md:self-auto border-t md:border-t-0 border-zinc-800/80 pt-4 md:pt-0">
          <button 
            ...
            title="편집 영상만 보기"
            class="bg-zinc-950/70 hover:bg-zinc-800/80 border ${currentTab === 'clip' ? 'border-red-500/80 ring-2 ring-red-500/20' : 'border-zinc-800/90'} rounded-2xl px-4 py-2.5 text-center min-w-[90px] transition-all cursor-pointer"
          >
          ...
          <button 
            onclick="setVideoTab('full')" 
            title="풀 영상만 보기"
            class="bg-zinc-950/70 hover:bg-zinc-800/80 border ${currentTab === 'full' ? 'border-indigo-500/80 ring-2 ring-indigo-500/20' : 'border-zinc-800/90'} rounded-2xl px-4 py-2.5 text-center min-w-[90px] transition-all cursor-pointer"
          >
    ...
    <!-- 영상 종류 선택 탭 바 (편집 영상 / 풀 영상) 및 어드민 추가 버튼 -->
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      <div class="flex items-center gap-3 flex-wrap">
        <div class="flex items-center gap-1.5 p-1 bg-zinc-900/90 border border-zinc-800 rounded-2xl w-fit">
          <button 
            onclick="setVideoTab('clip')" 
            class="flex items-center gap-2 px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition-all cursor-pointer ${currentTab === 'clip' ? 'bg-red-600 text-white shadow-lg shadow-red-600/30' : 'text-zinc-400 hover:text-white hover:bg-zinc-800/60'}"
          >
            <span>🎬 편집 영상</span>
          ...
          <button 
            onclick="setVideoTab('full')" 
            class="flex items-center gap-2 px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition-all cursor-pointer ${currentTab === 'full' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-zinc-400 hover:text-white hover:bg-zinc-800/60'}"
          >
            <span>📹 풀 영상</span>
        ...
        ${isAdmin() ? `
          <button 
            ...
            class="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-2xl bg-amber-500 hover:bg-amber-400 text-black text-xs font-bold transition-all shadow-lg shadow-amber-500/20 cursor-pointer"
            title="새 유튜브 영상 등록 (어드민)"
          >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
            <span>+ 영상 추가</span>
          </button>
      ...
      <div class="flex items-center gap-2 flex-wrap text-xs text-zinc-400">
        <span class="whitespace-nowrap flex-shrink-0">💡 썸네일을 누르면 유튜브 새 탭으로 즉시 열립니다</span>
      </div>
    </div>
...
function renderSearchResults(container) {
  ...
  KONGBAB_DATA.categories.forEach(cat => {
    if (cat.hasSubgroups) {
      cat.groups.forEach(group => {
        if (group.name.toLowerCase().includes(q) || (group.description && group.description.toLowerCase().includes(q))) {
  ...
  results.forEach(item => {
    if (item.type === 'video') {
      ...
      html += `
        <div 
          onclick="selectMemberFromSearch('${item.category.id}', ${item.group ? `'${item.group.id}'` : 'null'}, '${m.id}')"
          class="bg-zinc-900 border border-zinc-800 hover:border-amber-400/50 rounded-2xl p-5 cursor-pointer transition-all hover:-translate-y-1"
        >
      ...
      html += `
        <div 
          onclick="selectGroupFromSearch('${item.category.id}', '${g.id}')"
          class="bg-zinc-900 border border-zinc-800 hover:border-amber-400/50 rounded-2xl p-5 cursor-pointer transition-all hover:-translate-y-1"
        >
          <span class="text-[10px] font-bold px-2 py-0.5 rounded bg-zinc-800 text-zinc-300">[조직] ${item.category.name}</span>
          <h4 class="text-xl font-bold text-white mt-2 mb-1 flex items-center gap-1.5">
            <span>${g.emoji || ''}</span>
            <span>${g.name}</span>
          </h4>
          <p class="text-xs text-zinc-400 line-clamp-2 mb-3">${g.description}</p>
          <div class="pt-3 border-t border-zinc-800 flex justify-between items-center text-xs text-zinc-500">
